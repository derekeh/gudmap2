...
public class GenericQueries {
	  ...
	  // public final static String ASSAY_TYPE_TOTAL_GUDMAP_ACCESSION="SELECT COUNT(DISTINCT SUB_ACCESSION_ID) AS TOTAL FROM ISH_SUBMISSION %s WHERE SUB_ASSAY_TYPE = ? AND "+GenericQueries.PUBLIC_ENTRIES+ "%s %s";
	   ...
	   "GROUP_CONCAT(DISTINCT x.tissue) tissue, x.expression, x.specimen, x.image " +
		...
		"WHERE  ( SUB_ACCESSION_ID IN (%s) " +
		"OR RPR_MTF_JAX IN (%s) " +
		"OR CONCAT(RPR_PREFIX,RPR_OID) IN (%s)  " + 
		"OR RPR_LOCUS_TAG  IN (%s)  " + 
		"OR RPR_ENSEMBL  IN (%s)  ) " +
		"AND SUB_IS_PUBLIC = 1 AND SUB_IS_DELETED = 0 AND SUB_DB_STATUS_FK = 4  AND SUB_ASSAY_TYPE NOT IN ('Microarray','NextGen')   GROUP BY oid )  " +
		...
		"'' specimen, '' image " +
		"FROM ISH_SUBMISSION, ISH_EXPRESSION, ISH_SPECIMEN, ANA_NODE, ANA_TIMED_NODE  " +
		"WHERE SUB_ACCESSION_ID   IN (%s)   AND SUB_ASSAY_TYPE = 'Microarray' AND SUB_IS_PUBLIC = 1 AND SUB_IS_DELETED = 0 AND SUB_DB_STATUS_FK = 4 AND EXP_SUBMISSION_FK=SUB_OID AND " + 
		"SPN_SUBMISSION_FK=SUB_OID AND ATN_PUBLIC_ID = EXP_COMPONENT_ID AND ATN_NODE_FK = ANO_OID ) " +
		...
		"LEFT JOIN ISH_ALLELE ON SAL_ALE_OID_FK = ALE_OID WHERE SUB_ASSAY_TYPE = 'NextGen' AND SUB_IS_PUBLIC = 1 AND SUB_IS_DELETED = 0 AND SUB_DB_STATUS_FK = 4   " +
		"AND SUB_ACCESSION_ID  IN (%s)  ) " +
	   ...
	   public final static String ISH_ACCESSION_TOTAL = "SELECT COUNT(DISTINCT SUB_OID) TOTAL_INSITU FROM ISH_SUBMISSION JOIN ISH_PROBE ON SUB_OID = PRB_SUBMISSION_FK " +
			   "LEFT JOIN REF_PROBE ON RPR_OID = PRB_MAPROBE " +
			   "WHERE  ( SUB_ACCESSION_ID   IN (%s)  " +
				"OR RPR_MTF_JAX  IN (%s)  " +
				"OR CONCAT(RPR_PREFIX,RPR_OID)  IN (%s) " + 
				"OR RPR_LOCUS_TAG IN (%s) " + 
				"OR RPR_ENSEMBL  IN (%s)  ) " +
			   "AND SUB_IS_PUBLIC = 1 AND SUB_IS_DELETED = 0 AND SUB_DB_STATUS_FK = 4 AND SUB_ASSAY_TYPE NOT IN ('Microarray','NextGen')";
	   
	   public final static String MICROARRAY_ACCESSION_TOTAL = "SELECT COUNT(DISTINCT SUB_OID) TOTAL_MICROARRAY FROM ISH_SUBMISSION " +
			   "WHERE SUB_ACCESSION_ID  IN (%s)   AND SUB_ASSAY_TYPE = 'Microarray' AND SUB_IS_PUBLIC = 1 AND SUB_IS_DELETED = 0 AND SUB_DB_STATUS_FK = 4";
	   
	   public final static String SEQUENCE_ACCESSION_TOTAL = "SELECT COUNT(DISTINCT SUB_OID) TOTAL_SEQUENCE FROM ISH_SUBMISSION " +
			   "WHERE SUB_ACCESSION_ID  IN (%s)   AND SUB_ASSAY_TYPE = 'NextGen' AND SUB_IS_PUBLIC = 1 AND SUB_IS_DELETED = 0 AND SUB_DB_STATUS_FK = 4"; 
}
