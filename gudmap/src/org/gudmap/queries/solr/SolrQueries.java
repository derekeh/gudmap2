...
public class SolrQueries {
	...
	public final static String GET_GENE_INDEX_DATA = "SELECT SOLR_GENES_INDEX.*, PROBESETS, ENTREZ_ID, GENBANK_ID, " +
			...
			"SOURCE, PI_NAME, LAB, ANCHOR, MARKER FROM SOLR_GENES_INDEX LEFT JOIN SOLR_MA_GENES ON (GENE=MA_GENE) " +
	...
	public final static String GET_GENELISTS_INDEX_DATA = "SELECT GNL_OID AS ID,GNL_SHORT_NAME AS NAME,GNL_DESCRIPTION AS DESCRIPTION," +
			...
			"GNL_STAGE AS THEILER_STAGE,GNL_SEX AS SEX,GNL_GENELIST_TYPE AS GENELIST_TYPE,GROUP_CONCAT(DISTINCT GLE_ENTITY_ID SEPARATOR ';') " +
	...
	public final static String GET_INSITU_INDEX_DATA = "SELECT SOLR_INSITU_DETAILS.*,SOLR_INSITU_EXP.PRESENT AS PRESENT,SOLR_INSITU_EXP." +
			...
			"SOLR_INSITU_EXP.ANNOTATION AS ANNOTATION,SOLR_INSITU_EXP.TISSUE_TYPE AS TISSUE_TYPE FROM SOLR_INSITU_DETAILS " +
	...
	public final static String GET_SAMPLES_INDEX_DATA = "SELECT DISTINCT substring(SUB_ACCESSION_ID,8) AS GUDMAP," +
			"SMP_GEO_ID AS SAMPLE_GEO_ID,CONCAT('TS',SMP_THEILER_STAGE) AS THEILER_STAGE, " +
			"TRIM(CASE SPN_STAGE_FORMAT WHEN 'dpc' THEN CONCAT(SPN_STAGE,' ',SPN_STAGE_FORMAT) WHEN 'P' THEN CONCAT('P',SPN_STAGE) " +
			"ELSE CONCAT(SPN_STAGE_FORMAT,SPN_STAGE) END) AS STAGE, SUB_SOURCE AS SOURCE, IF ((SELECT COUNT(*) " +
			...
			"PLT_GEO_ID AS PLATFORM_GEO_ID FROM ISH_SUBMISSION JOIN MIC_SAMPLE ON SMP_SUBMISSION_FK = SUB_OID " +
	...
	public final static String GET_TISSUE_INDEX_DATA = "SELECT A.CSC_OID AS ID, A.CSC_DISPLAY_NAME AS NAME, A.CSC_SYNONYM AS SYNONYM, " +
			...
			"SOLR_TS_EMAPIDS.TS_THEILER_STAGE AS THEILER_STAGE FROM CPS_COMPONENTS A " +
	...
	public final static String GET_NEXTGEN_SAMPLES_INDEX_DATA = "SELECT DISTINCT substring(SUB_ACCESSION_ID,8) AS GUDMAP, " +
			...
			"CONCAT(NGS_STAGE_FORMAT,NGS_DEV_STAGE) END) AS AGE,SUB_SUB_DATE AS DATE,NGS_SEX AS SEX,NGS_DESCRIPTION AS SAMPLE_DESCRIPTION," +
