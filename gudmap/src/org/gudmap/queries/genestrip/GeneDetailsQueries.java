...
public class GeneDetailsQueries {
	  ...
	  public static String ALTERNATIVE_GENE_INFO = "SELECT DISTINCT RMM_SYMBOL, RMM_NAME, RMM_MGIACC, (SELECT GROUP_CONCAT(RSY_SYNONYM SEPARATOR '|') FROM REF_SYNONYM " +
			  "WHERE RSY_MGIACC_FK= ? ORDER BY RSY_SYNONYM) SYNONYMS, CASE substring(RMM_MGIACC from 1 for position(':' in RMM_MGIACC)) WHEN 'MGI:' THEN " +
			  "CONCAT(GENE_URL.URL_URL, RMM_MGIACC) ELSE /* NON MGI */ '' END, " +
			  "REG_STABLE, CONCAT(ENS_URL.URL_URL,REG_STABLE), CASE substring(RMM_MGIACC from 1 for position(':' in RMM_MGIACC)) WHEN 'MGI:' THEN " +
			  "CONCAT(GO_URL.URL_URL, RMM_MGIACC) ELSE /* non-MGI */ '' END, " +
			  "CONCAT(OMIM_URL.URL_URL,RMM_SYMBOL), CONCAT(ENTREZ_URL.URL_URL,RMM_SYMBOL), REG_CHROM_START, REG_CHROM_END, REG_CHROME_NAME, M.MIS_ENS_GENEBUILD, " +
			  "CONCAT(GENECARDS_URL.URL_URL,RMM_SYMBOL,GENECARDS_URL.URL_SUFFIX), CONCAT(HGNC_SYMBOL_SEARCH_URL.URL_URL,RMM_SYMBOL), CONCAT(UCSC_URL.URL_URL, REG_CHROME_NAME, ':', " +
			  "REG_CHROM_START, '-', REG_CHROM_END, UCSC_URL.URL_SUFFIX), RMM_SPECIES " +
			  "FROM " +
			  "REF_MISC M, REF_MGI_MRK LEFT JOIN REF_ENS_GENE ON REG_PRIMARY_ACC = RMM_MGIACC " +
			  "JOIN REF_URL GENE_URL " +
			  "JOIN REF_URL GO_URL " +
			  "JOIN REF_URL ENS_URL " +
			  "JOIN REF_URL OMIM_URL " +
			  ...
			  "JOIN REF_URL UCSC_URL " +
			  "WHERE " +
			  "GENE_URL.URL_TYPE = 'jax_gene' AND GO_URL.URL_TYPE = 'go_gene' AND ENS_URL.URL_TYPE = 'ens_gene' AND OMIM_URL.URL_TYPE = 'omim_gene' AND ENTREZ_URL.URL_TYPE = 'entrez_all' " +
			  "AND GENECARDS_URL.URL_TYPE = 'genecards_gene' AND HGNC_SYMBOL_SEARCH_URL.URL_TYPE = 'hgnc_symbol_search' AND UCSC_URL.URL_TYPE = 'ucsc_genome'  " +
			  "AND RMM_MGIACC = ?";
