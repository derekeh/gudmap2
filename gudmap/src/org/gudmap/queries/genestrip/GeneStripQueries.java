...
//	  }
	  ...
	  /*public static String GENESTRIP_ROW_MINUS_PROFILES_2 = "SELECT DISTINCT RPR_LOCUS_TAG mgi, " +			    
		        ...
		        "(SELECT COUNT(DISTINCT DGA_OMIMID) FROM LNK_GENE_OMIMDIS WHERE DGA_MGIACC = mgi) omim FROM REF_PROBE WHERE RPR_LOCUS_TAG IN (%s) " +
		        "UNION " +
		        "SELECT DISTINCT GNF_ID mgi, " +
		        "(SELECT GROUP_CONCAT(RSY_SYNONYM) FROM REF_SYNONYM JOIN REF_MGI_MRK ON RSY_REF = RMM_ID WHERE RMM_MGIACC = mgi) synonyms, " +
		       // "(SELECT DISTINCT GNF_SYMBOL FROM REF_GENE_INFO WHERE GNF_ID= mgi) gene, " +
		        "(SELECT DISTINCT RMM_SYMBOL FROM REF_MGI_MRK,REF_GENE_INFO  WHERE RMM_MGIACC = GNF_ID AND GNF_ID= mgi) gene, " +
		        "(SELECT CONCAT(MIN(STG_ORDER),'-', MAX(STG_ORDER)) FROM MIC_BROWSE_CACHE JOIN REF_STAGE ON STG_OID = MBC_SUB_STAGE_FK WHERE MBC_MAN_MGI_ID = mgi) arrayRange, " +
		        "(SELECT CONCAT(MIN(STG_ORDER),'-', MAX(STG_ORDER)) " +
		        "FROM ISH_SUBMISSION, REF_STAGE, ISH_PROBE,REF_PROBE WHERE STG_OID = SUB_STAGE_FK AND PRB_SUBMISSION_FK=SUB_OID AND PRB_MAPROBE=RPR_OID AND RPR_LOCUS_TAG = mgi) ishRange, " +
		        "(SELECT DISTINCT STG_SPECIES " +
		        "FROM ISH_SUBMISSION, REF_STAGE, ISH_PROBE,REF_PROBE WHERE STG_OID = SUB_STAGE_FK AND PRB_SUBMISSION_FK=SUB_OID AND PRB_MAPROBE=RPR_OID AND RPR_LOCUS_TAG = mgi) species, " +
		        "(SELECT COUNT(DISTINCT DGA_OMIMID) FROM LNK_GENE_OMIMDIS WHERE DGA_MGIACC = mgi) omim FROM REF_GENE_INFO WHERE GNF_ID IN (%s) " +
		        "ORDER BY %s %s LIMIT ?, ?";*/
	  ...
	  public static String FIND_SPECIES_FOR_GENE_ID = "SELECT DISTINCT STG_SPECIES species FROM ISH_SUBMISSION, REF_STAGE, ISH_PROBE,REF_PROBE WHERE STG_OID = SUB_STAGE_FK AND PRB_SUBMISSION_FK=SUB_OID " +
			  "AND PRB_MAPROBE=RPR_OID AND RPR_LOCUS_TAG = ?";
	  
	  /*public static String GENESTRIP_TOTALS = "SELECT COUNT(DISTINCT RPR_LOCUS_TAG) mgi FROM REF_PROBE WHERE RPR_LOCUS_TAG IN (%s)";*/
	  
	 /* public static String GENESTRIP_TOTALS = "SELECT MAX(mgi) FROM (SELECT COUNT(DISTINCT RPR_LOCUS_TAG) mgi FROM REF_PROBE WHERE RPR_LOCUS_TAG IN (%s) "+
			  "UNION " +
			  "SELECT COUNT(DISTINCT GNF_ID) AS mgi FROM REF_GENE_INFO WHERE GNF_ID IN (%s)) as mgi";*/
	  
	  public static String GENESTRIP_TOTALS = "SELECT count(distinct mgi) FROM ( SELECT COUNT(DISTINCT RPR_LOCUS_TAG) as mgi FROM REF_PROBE WHERE RPR_LOCUS_TAG IN (%s) " +
			  "UNION " +
			  "SELECT COUNT(DISTINCT GNF_ID) AS mgi FROM REF_GENE_INFO WHERE GNF_ID IN (%s)) x";
