...
//	  }
	  ...
	  public static String GENESTRIP_ROW_MINUS_PROFILES = "SELECT ? gene,  " +
		  	    "(SELECT GROUP_CONCAT(RSY_SYNONYM) FROM REF_SYNONYM JOIN REF_MGI_MRK ON RSY_REF = RMM_ID WHERE BINARY RMM_SYMBOL = gene) synonyms, "+
		  	    "(SELECT DISTINCT RPR_LOCUS_TAG FROM REF_PROBE WHERE BINARY RPR_SYMBOL=gene) mgi, " +
		  		"(SELECT CONCAT(MIN(STG_ORDER),'-', MAX(STG_ORDER)) FROM MIC_BROWSE_CACHE JOIN REF_STAGE ON STG_OID = MBC_SUB_STAGE_FK WHERE BINARY MBC_GNF_SYMBOL=gene) arrayRange, " +
		        "(SELECT CONCAT(MIN(STG_ORDER),'-', MAX(STG_ORDER)) " +
		  		"FROM ISH_SUBMISSION, REF_STAGE, ISH_PROBE,REF_PROBE WHERE STG_OID = SUB_STAGE_FK AND PRB_SUBMISSION_FK=SUB_OID AND PRB_MAPROBE=RPR_OID AND BINARY RPR_SYMBOL=gene) ishRange, " +
		  		"(SELECT DISTINCT STG_SPECIES " +
		  		"FROM ISH_SUBMISSION, REF_STAGE, ISH_PROBE,REF_PROBE WHERE STG_OID = SUB_STAGE_FK AND PRB_SUBMISSION_FK=SUB_OID AND PRB_MAPROBE=RPR_OID AND BINARY RPR_SYMBOL=gene) species, " +
		        "(SELECT COUNT(DISTINCT DGA_OMIMID) FROM LNK_GENE_OMIMDIS WHERE BINARY DGA_SYMBOL=gene) omim";
